...
from datasets.dataset import DenseFusionDataset
...
parser.add_argument('--resume_posenet',
                    type=str,
                    default='',
                    help='resume PoseNet model')
parser.add_argument('--resume_refinenet',
                    type=str,
                    default='',
...
def main():
    ...
    if opt.dataset == 'ycbv':
        ...
        opt.outf = 'trained_models/ycbv'  #folder to save trained models
        opt.log_dir = 'experiments/logs/ycbv'  #folder to save logs
        opt.repeat_epoch = 1  #number of repeat times for one epoch training
    elif opt.dataset == 'lm':
        opt.num_objects = 15
        opt.num_points = 500
        opt.outf = 'trained_models/lm'
        opt.log_dir = 'experiments/logs/lm'
        opt.repeat_epoch = 20
    else:
    ...
    if opt.dataset == 'ycbv':
        points_num = (1000, 500, 2600)
        sym_list = [12, 15, 18, 19, 20]
    elif opt.dataset == 'lm':
        points_num = (500, 500, 500)
        sym_list = [7, 8]

    dataset = DenseFusionDataset(opt.dataset_root, opt.dataset, 'train',
                                 points_num, opt.noise_trans, sym_list)
    dataloader = torch.utils.data.DataLoader(dataset,
                                             batch_size=1,
                                             shuffle=True,
                                             num_workers=opt.workers)
    dataset_test = DenseFusionDataset(opt.dataset_root, opt.dataset, 'test',
                                      points_num, None, sym_list)
    testdataloader = torch.utils.data.DataLoader(dataset_test,
                                                 batch_size=1,
                                                 shuffle=False,
                                                 num_workers=opt.workers)

    opt.sym_list = dataset.get_sym_list()
    opt.num_points_mesh = dataset.get_num_points_model()

    print(
        '>>>>>>>>----------Dataset loaded!---------<<<<<<<<\nlength of the training set: {0}\nlength of the testing set: {1}\nnumber of sample points on mesh: {2}\nsymmetry object list: {3}'
        .format(len(dataset), len(dataset_test), opt.num_points_mesh,
                opt.sym_list))
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                if has_cuda:
                    ...
                    points, choose, img, target, model_points, idx = Variable(points), \
                                                                    Variable(choose), \
                                                                    Variable(img), \
                                                                    Variable(target), \
        ...
        if best_test < opt.refine_margin and not opt.refine_start:
            ...
            dataloader = torch.utils.data.DataLoader(dataset,
                                                     ...
                                                     num_workers=opt.workers)
            if opt.dataset == 'ycb':
                dataset_test = PoseDataset_ycb('test', opt.num_points, False,
                                               opt.dataset_root, 0.0,
                                               opt.refine_start)
            elif opt.dataset == 'linemod':
                dataset_test = PoseDataset_linemod('test', opt.num_points,
